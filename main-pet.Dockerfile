...
ENV Packages "coreutils findutils nc sudo libuser tmux zsh gpg wget rsync \
              passwd sqlite pinentry git \
              neovim python-neovim python3-neovim gcc gcc-c++ make \
              gtk3-devel meson ncurses-compat-libs gstreamer1*devel"
# to get gpg-agent to work it is necessary to create the /run/user/1000
# directory
# dnf upgrade fails installing "setup" package so it is necessary to 
# avoid breaking the sequence of commands
...
# set entrypoint to always launch tmux (it is a pet container so it is
